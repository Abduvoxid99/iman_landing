...
import { Box, Container, Typography, Slider, Grid } from "@material-ui/core";
import cls from "./Invest.module.scss";
...
export default function Invest() {
  ...
  const investedNumber = 12 * years * valueMonth + value;
    ...
    const array1 = investTotal.map((el, ind) => ({
      name:
        Math.floor(ind / 12) > 0 && ind + 1 !== putInvest.length
          ? ""
          : Math.floor(ind / 12),
      Total_earned: (el + putInvest[ind]).toString(),
  ...
  // console.log("PUT Invest => ", putInvest)
  ...
  return (
    <div className={cls.root} id="invest">
      {/* <img className={`bgColor left ${cls.figure}`} src='/images/bg-icon.png' /> */}
              ...
              <Fade right>
                <div className={cls.rightCard}>
                  <div className={cls.upCard}>
                    ...
                    <span className={cls.upCardBottom}>{t("calculation")}</span>
                  </div>
                  <div className={cls.downCard}>
                    {data?.length && (
                      <ResponsiveContainer
